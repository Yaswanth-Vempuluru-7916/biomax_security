#!/usr/bin/env python3
"""
Final Python Attendance Data Extractor
Exact match to the working sample application with correct function signatures
"""
...
# Numeric ID function (original) - uses int for datetime
fk_dll.FK_GetGeneralLogData.argtypes = [
    ...
    ctypes.POINTER(ctypes.c_int),     # apnDateTime (int format)
    ctypes.POINTER(ctypes.c_int)      # apnWorkCode
]
fk_dll.FK_GetGeneralLogData.restype = ctypes.c_int

# String ID function - uses DateTime for datetime (like sample app)
# We need to create a custom DateTime structure
class DateTime(ctypes.Structure):
    _fields_ = [
        ...
        ("minute", ctypes.c_uint16),
        ("second", ctypes.c_uint16),
        ("day_of_week", ctypes.c_uint16)
    ]
...
fk_dll.FK_GetGeneralLogData_StringID_Workcode.argtypes = [
    ...
    ctypes.POINTER(ctypes.c_int),     # apnVerifyMode
    ctypes.POINTER(ctypes.c_int),     # apnInOutMode
    ctypes.POINTER(DateTime),         # apnDateTime (DateTime structure)
    ctypes.POINTER(ctypes.c_int)      # apnWorkCode
]
...
def convert_datetime_struct(dt_struct: DateTime) -> str:
    """Convert DateTime structure to formatted string"""
    try:
        # Format as 2025/08/06 08:25:22
        return f"{dt_struct.year}/{dt_struct.month:02d}/{dt_struct.day:02d} {dt_struct.hour:02d}:{dt_struct.minute:02d}:{dt_struct.second:02d}"
    except:
        return f"{dt_struct.year}-{dt_struct.month}-{dt_struct.day} {dt_struct.hour}:{dt_struct.minute}:{dt_struct.second}"
...
def extract_attendance_data() -> Dict[str, Any]:
    ...
    try:
        ...
        if use_string_id:
            ...
            sdk_datetime = DateTime()  # DateTime structure
            ...
            while True:
                ...
                if get_result == 1:  # RUN_SUCCESS
                    ...
                    record = {
                        "UserId": user_id,
                        "LogTime": convert_datetime_struct(sdk_datetime),
...
def main():
    """Main function"""
    print("=== Final Python Attendance Data Extractor ===")
    print("Exact match to working sample application")
