...
namespace FKRealSvrOcxTcpSample
{
    public partial class Form1 : Form
    {
        ...
        private void AxAxImage1_Enter(object sender, EventArgs e)
        {
        ...
        private void btnBsCs_Click(object sender, EventArgs e)
        {
            ...
            string vpszIPAddress = (txtIPAddress.Text).Trim();
            string serverIP = txtHostIP.Text.Trim();
            string serverPort = txtHostPort.Text.Trim();
            int vpszNetPort = Convert.ToInt32(txtPortNo.Text);

            int vpszNetPassword = Convert.ToInt32(txtPassword.Text);
            int vnTimeOut = 5000;
            int vnProtocolType = (int)enumProtocolType.PROTOCOL_TCPIP;
            //建立连接
            FKAttendDLL.nCommHandleIndex = FKAttendDLL.FK_ConnectNet(vnMachineNumber, vpszIPAddress, vpszNetPort, vnTimeOut, vnProtocolType, vpszNetPassword, 1261);
            if (FKAttendDLL.nCommHandleIndex > 0)
            {
                string cmdStr = "{ \"cmd\":\"cs_serverinfo_set\",\"param\":{ \"server_ip\":\"" + serverIP + "\",\"server_port\":\"" + serverPort + "\"} }";
                //发送设置指令
                FKAttendDLL.nCommHandleIndex = FKAttendDLL.FK_HS_ExecJsonCmd(vnMachineNumber, ref cmdStr);
                if (FKAttendDLL.nCommHandleIndex > 0)
                    MessageBox.Show("设置成功");
                else
                    MessageBox.Show("设置失败");
                //关闭连接
                FKAttendDLL.FK_DisConnect(vnMachineNumber);
            }
            else
            {
